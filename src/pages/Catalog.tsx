...
import { SlidersHorizontal, RefreshCw, ChevronDown, ChevronUp, X } from "lucide-react";
...
export const Catalog = () => {
    ...
    const filteredProducts = products.filter(product => {
        ...
        const matchsCategory = currentCategory
            ? product.category.toLowerCase() === currentCategory.toLocaleLowerCase()
            : true;
        ...
        return matchsCategory && matchesPrice && matchesStock;
    });
        ...
        <div className="min-h-screen bg-nexus-bg text-nexus-text-main pt-8 pb-16 px-4 md:px-8">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-nexus-surface border border-nexus-border/40 p-4 rounded-xl mb-8 ">
                    ...
                    {(currentCategory || onlyWithStock || maxPrice < 500000) && (
                        <span className="bg-nexus-brand/10 text-nexus-brand border border-nexus-brand/30 text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md flex items-center gap-1.5 animate-fade-in">
                            {currentCategory}
                            <X size={12} className="cursor-pointer hover:text-white" onClick={handleClearFilters} />
                        </span>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
                    ...
                    {/* Columna izquierda.- */}
                    <aside className="grid grid-cols-2 lg:grid-cols-1 gap-4 lg:sticky lg:top-24">
                        <div className="flex flex-wrap justify-between items-center bg-nexus-surface/50 border border-nexus-border/40 px-4 py-3 rounded-xl">
                        ...
                        {/* Filtro: Categorias.- */}
                        <div className="bg-nexus-surface border border-nexus-border/60 rounded-xl overflow-hidden">
                            <span className="text-[10px] font-black uppercase tracking-wider text-nexus-text-muted block p-2">Categorías</span>

                            {["Teclados", "Mouse", "Audio"].map((cat) => (
                                <button
                                    key={cat}
                                ...
                                <div className="pt-1 animate-fade-in">
                                    <label className="flex items-center gap-3 cursor-pointer group text-sm text-nexus-text-main select-none">
                                        <div className="relative flex items-center">
                                            ...
                                            />
                                            {/* Checkbox customizado estilo Nexus */}
